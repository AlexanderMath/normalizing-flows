...
parser.add_argument('-densityEstimation', '--densityEstimation', dest='densityEstimation',
                    default=True, action='store_true', help='perform density estimation')
...
def main():
    ...
    for epoch in range(1, 1+args.epochs):
        ...
        train(args, model, optimizer, epoch, trainloader, trainset, use_cuda)
    ...
    print('Testing model')
    test_log = open(os.path.join(args.save_dir, "test_log.txt"), 'w')
    test_objective = test(test_objective, args, model, epoch, testloader, use_cuda)
